...
public class Point {
    ...
    public boolean isPointInPolygon(double px, double py,
                                    List<String> polygonXA, List<String> polygonYA) {
        ...
        for (int i = 0; i < polygonXA.size() - 1; i++) {
            double cx1 = Double.parseDouble(polygonXA.get(i));
            double cy1 = Double.parseDouble(polygonYA.get(i));
            double cx2 = Double.parseDouble(polygonXA.get(i + 1));
            double cy2 = Double.parseDouble(polygonYA.get(i + 1));
            // 如果目标点在任何一条线上
